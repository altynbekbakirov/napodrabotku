...
class OtpVerificationScreen extends StatefulWidget {
  OtpVerificationScreen({Key key, @required this.verificationId, @required this.users, @required this.phone, this.imageFile, this.login = false})
      : super(key: key);
          ...
          try {
            if (widget.login) {
              ...
              if (isSubmitEnabled) {
                ...
                final credential = PhoneAuthProvider.credential(verificationId: widget.verificationId, smsCode: smsCode);
                await auth.signInWithCredential(credential).then((value) async {
                  await Users.checkUsername(widget.users.email).then((value) async {
                    /// Validate form
                    ...
                    request.fields["linkedin"] = widget.users.linkedin;
                    request.fields["is_migrant"] = widget.users.is_migrant.toString();
                    ...
                    request.fields["job_type"] = widget.users.job_type.toString();
                    request.fields["is_product_lab_user"] = widget.users.is_product_lab_user ? "1" : "0";
